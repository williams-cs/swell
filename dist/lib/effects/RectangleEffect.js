...
class RectangleEffect {
    ...
    modifyResize(widthTooSmall, heightTooSmall) {
        if (widthTooSmall) {
            ...
            if (newDistance - this._initDistance > 0) {
                this.modifyResizeHelper(newDistance, true);
            }
        }
        if (heightTooSmall) {
            ...
            let newDistance = distance(this._mouse.x, this._mouse.y, this._dragoffx, this._dragoffy);
            this.modifyResizeHelper(newDistance, false);
        }
    }
    modifyResizeHelper(newDistance, isTooSmall) {
        if (!isTooSmall) {
            switch (this._corner) {
                case 1:
    ...
    modifyChangeDims(widthTooSmall, heightTooSmall) {
        ...
        if (widthTooSmall) {
            ...
            if (newDistance - this._initDistance > 0) {
                this.modifyChangeDimsHelper(true);
            }
        }
        if (heightTooSmall) {
            this._dims.height.eval(this._context).val = 10;
            ...
            this.modifyChangeDimsHelper(false);
        }
    }
    modifyChangeDimsHelper(isTooSmall) {
        let newDistance = distance(this._mouse.x, this._mouse.y, this._dragoffx, this._dragoffy);
        switch (this._corner) {
            case 5:
                if (!isTooSmall) {
