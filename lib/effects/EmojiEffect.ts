...
export class EmojiEffect implements Effect<EmojiNode> {
    ...
    logResize(): LogEvent<any> {
        return new ResizeEvent("emoji with ID " + this.getID().toString(), Math.round(this._width1*100)/100, Math.round(this._height1*100)/100, Math.round(this.w*100)/100, Math.round(this.h*100)/100);
    }
    ...
    logClick(): LogEvent<any>{
        return new ClickEvent("emoji with ID " + this.getID().toString(), this.x, this.y);
    }
